...
"""
    ...
    async def execute(self, query):
        ...
        result = await get_model_response(
            model_name="deepseek-chat",  # deepseek-reasoner deepseek-chat
            messages=messages,
            stream=True
        )
        all_answer = ""
...
## Goals：
- 你需要分析用户的问题，和已有的信息，紧盯着原始问题，根据现有情况作出下一步的action，直到问题解决。
如果有满足需求的工具，优先使用工具来解决问题（tools中的工具就是你可以使用的全部工具）；如果没有工作，则你自己进行作答即可。

## tools:
{tools}

## Constraints：
- 你必须使用“@self:”的格式来触发自己的回答。
- 当需要调用工具的时候，你需要使用"=>$tool_name: {key:value}"的格式来调用工具，其中参数为严格的json格式，例如"=>$send_email: {subject: 'Hello', content: 'This is a test email'}"。
- 每一次触发了不同的tool之后，你需要停止作答，等待用户调用对应的tool处理之后，将tool的结果重新组织语言后再继续作答，新的答案要接着“=>$tool_name”前面的最后一个字符继续生成结果，要保持结果通顺。
...
## Goals：
你需要分析用户的问题，根据现有工具情况，给出解决该问题的步骤（tools中的工具就是你可以使用的全部工具）。如果某些步骤没有合适的工具，则该步骤你自己回答（该步骤可以标识为@self）
