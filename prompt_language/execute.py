...
1. 变量标识符
- 变量以数字字母下划线组成并且不能以数字开头，要用$variable表示，如果变量后有其它数字、字母、下划线，需要在末尾加空格。
...
- 标识符：FOR - END
- 规范：以大写的FOR开头，大写END结尾。必须有明确的缩进（4个空格）
- examples：
...
- 规范：以大写的IF开头，大写END结尾。必须有明确的缩进（4个空格）
...
灵活的注入agent，流畅、轻量实现各种任务

- 标识符：@agent_init(roles={}, tools={}) -> agent_name

- 规范：以@agent_init开头，赋值语句结束。初始化后的agent会作为工具注册进全局变量池。
...
- 标识符：@agent(agent_name="default", task="查一下上海的天气，根据天气情况写一首诗，并将结果发送到我的邮箱", tools)-> res

- 参数agent_name：可以指定@agent_init初始化的agent，也可以不指定。若不指定，则调用默认agent模式。

- 参数task：用户任务，自然语言表述即可。

- 参数tools：可以指定tools的范围，若不指定，则使用全局变量池中的所有工具。

- 规范：以 @agent 开头，赋值语句结尾。
